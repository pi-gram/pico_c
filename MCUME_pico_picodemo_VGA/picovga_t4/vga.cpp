...
void VgaDmaInit()
{
	printf("VGA.CPP: about to VgaDmaInit\n");
...
void VgaPioInit()
{
	...
	printf("VGA.CPP: about to VgaPioInit\n");
	...
	printf("VGA.CPP: about to init GPIO pins\n");
        //TP - pretty sure this is where I need to modify it to not just go 0-x but do the proper GPIO inits

	pio_gpio_init(VGA_PIO, VGA_GPIO0_R0);
	pio_gpio_init(VGA_PIO, VGA_GPIO1_R1);
	pio_gpio_init(VGA_PIO, VGA_GPIO2_R2);
	pio_gpio_init(VGA_PIO, VGA_GPIO3_R3);
	pio_gpio_init(VGA_PIO, VGA_GPIO4_R4);

	pio_gpio_init(VGA_PIO, VGA_GPIO6_G0);
	pio_gpio_init(VGA_PIO, VGA_GPIO7_G1);
	pio_gpio_init(VGA_PIO, VGA_GPIO8_G2);
	pio_gpio_init(VGA_PIO, VGA_GPIO9_G3);
	pio_gpio_init(VGA_PIO, VGA_GPIO10_G4);

	pio_gpio_init(VGA_PIO, VGA_GPIO11_B0);
	pio_gpio_init(VGA_PIO, VGA_GPIO12_B1);
	pio_gpio_init(VGA_PIO, VGA_GPIO13_B2);
	pio_gpio_init(VGA_PIO, VGA_GPIO14_B3);
	pio_gpio_init(VGA_PIO, VGA_GPIO15_B4);

	// connect PIO to the pad
//	for (int i = VGA_GPIO_FIRST; i < VGA_GPIO_LAST; i++) {
//	    pio_gpio_init(VGA_PIO, i);
//	    printf("VGA.CPP: initialised GPIO: %d\n",i);
//	}
	printf("VGA.CPP: about to init VGA_GPIO_SYNC %d\n", VGA_GPIO_SYNC);	
	pio_gpio_init(VGA_PIO, VGA_GPIO_SYNC);
	
//	printf("VGA.CPP: what does this contain? %d\n", VGA_SYNCBASE);
#if VGA_VSYNC
	printf("VGA.CPP: and do we do this as well?\n");
	gpio_init(VGA_VSYNC);
#endif
	...
	if (!CurVmode.psync) gpio_set_outover(VGA_GPIO_SYNC, GPIO_OVERRIDE_INVERT);
        ...
        //TP - replace the range set to OUT (as GPIO5 is used for SDCARD)
...
#if VGA_VSYNC
	printf("VGA.CPP: and do we do this as well as well?\n");
...
//	//TP - comment this out and do it in 2 blocks
...
//	sm_config_set_out_pins(&cfg, VGA_GPIO_FIRST, VGA_GPIO_OUTNUM);
	...
	printf("VGA.CPP: should have finished the VGAPioInit\n");
}

// initialize scanline buffers
void VgaBufInit()
{
	printf("VGA.CPP: about to VgaBufInit\n");
...
void VgaTerm()
{
	printf("VGA.CPP: about to VgaTerm\n");
...
{
	printf("VGA.CPP: about to VgaInit\n"); //not 100% sure what calls this from within ::begin() ?
	...
	// initialize VGA PIO
	VgaPioInit(); //this is where we init and set the GPIO Pins
