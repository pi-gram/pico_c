...
int emu_ReadKeys(void) 
{
  ...
  emu_printf("I am inside emu_Readkeys...waiting...");
  ...
  if ((time_ms-last_t_ms) > 100) {
    ...
    if (ledflash_toggle == false) {
      ...
      emu_printf("ledflash TRUE");
    }
    else {
      ledflash_toggle = false;
      emu_printf("lefflash FALSE");
    }  
  }  
 
  if ( alt_pressed ) {
    emu_printf("alt was pressed");
    if (key_fn == false) 
    {
      ...
      emu_printf("something other than alt was pressed");
      // Keep press
      if (hundred_ms_cnt == 1) {
  ...
  // Handle LED
  if (key_alt == true) {
    emu_printf("we are flashing the LED");
...
#endif  
//TP - might need to spoof us pressing a button here?
  retval |= MASK_KEY_USER1;  //0x0020
  emu_printf("returning retval");
  emu_printf(retval);
...
int emu_ReadI2CKeyboard(void) {
  int retval=0;
#ifdef PICOMPUTER
//TP - this is where I need to map 3 buttons / keys?
  if (key_alt) {
...
//not needed as we removed the /sd_driver folder code
...
static int readNbFiles(char * rootdir) {
  ...
  while ( (fr == FR_OK) && (entry.fname[0]) && (totalFiles<MAX_FILES) ) {  
    ...
    //TP - added to see if it read a file name?
    emu_printf("..and the filename is:");
    emu_printf(entry.fname);
...
int handleMenu(uint16_t bClick)
{
  ...
  //if bClick and joystick BTN pressed?
  //selects the highlighted file on the screen
  ...
  } //if bClick and key pressed by user then =...
  ...
  //else just do a menuRedraw and show the list of files
  menuRedraw=true;
  action = ACTION_RUNVGA;
//comment out whole if statement!
  //if (menuRedraw && nbFiles) {
    int fileIndex = 0;
    //this should be listing out the filenames onto the screen
...
void toggleMenu(bool on) {
  if (on) {
    menuOn = true;
    backgroundMenu(); //this outputs the TITLE
  } else {
    menuOn = false;    
  }
}
...
void emu_init(void)
{
  emu_printf("emu_init start");
  
  // Dual display config, initialize TFT
//#if (defined(ILI9341) || defined(ST7789)) && defined(USE_VGA)
//  tft.begin();
//#endif
//force the TFT to start?

//I am pretty sure this has already been called in pico800.cpp before calling this function
//  emu_printf("redoing tft.begin");
//  tft.begin();
  //above debug outputs
//18
//19
...
//  emu_printf("about to init the sd card driver in SDCARD.C");
//  sd_init_driver(); 
  emu_printf("about to mount the SD card");
//  FRESULT fr = f_mount(&fatfs, "0:", 1);
  FRESULT fr = f_mount(&fatfs, "", 1);
  emu_printf("f_mount called and could be a success");
...
/*
  ...
  emu_printf("this loops a lot waiting for a key to be pressed");  
  int keypressed = emu_ReadKeys();
  
  emu_printi(keypressed);
  emu_printf("a key was pressed?");
#ifdef PICOMPUTER
...
*/  
  emu_printf("about to attempt to write out to the screen");
  tft.fillScreenNoDma( RGBVAL16(0xff,0x00,0x00) );
  tft.drawTextNoDma(64,48,    (char*)" TONY OUTPUT", RGBVAL16(0xff,0xff,0x00), RGBVAL16(0xff,0x00,0x00), true);
  tft.drawTextNoDma(64,48+24, (char*)"We'll need to reset now!", RGBVAL16(0xff,0xff,0x00), RGBVAL16(0xff,0x00,0x00), true);

  emu_printf("apparently about to toggle menu");
  toggleMenu(true); //this currently outputs the TITLE at the top of the screen
}
