...
static void Initialise(void)
{
  ...
  emu_printf("ATARI800.c: Initialising ...");
...
void at8_Init(void)
{
  ...
  emu_printf("ATARI800.c: Allocating RAM");
...
void at8_Step(void)
{
  emu_printf("ATARI800.c: step");

  int j=emu_ReadKeys();
  int k=emu_GetPad();
  int hk = emu_ReadI2CKeyboard();  

  emu_printf("j="); emu_printf(j);
  emu_printf("k="); emu_printf(k);
  emu_printf("hk="); emu_printf(hk);
...
void at8_Start(char * cartname)
{
  ...
  emu_printf("ATARI800.c: antic");
  ANTIC_Initialise();
  emu_printf("ATARI800.c: gtia");  
  GTIA_Initialise();
  emu_printf("ATARI800.c: pia");
  PIA_Initialise();  
  emu_printf("ATARI800.c: pokey");
  POKEY_Initialise();
  PORTA = 0x00;
  ANTIC_NMIEN = 0x00;
  ANTIC_NMIST = 0x00;
  ...
  GTIA_TRIG[2]=1;
  GTIA_TRIG[3]=1;
  emu_printf("ATARI800.c: 6502 reset");
  CPU_Reset();
    
  emu_printf("ATARI800.c: init done");   
}
